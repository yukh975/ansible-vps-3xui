...
> `group_vars/new_vps.yml` содержит **общие** переменные для всех хостов в группе (пользователи, ssh_port, пакеты, sysctl). Индивидуальные параметры (как минимум `hostname` — для ACME-сертификата) выносятся в `host_vars/<имя_хоста>.yml`. См. раздел «Развёртывание на несколько серверов» в [README_RU.md](README_RU.md) и шаблон `host_vars/server1.yml.example`.
...
> 💡 **Несколько серверов сразу?** Добавьте строки в секцию `[new_vps]` и создайте `host_vars/<host>.yml` для каждого (с уникальным `hostname`). Ansible развернёт параллельно. Подробности — в [README_RU.md](README_RU.md).
...
> Подробнее — в [README_RU.md](README_RU.md), callout в самом начале.
